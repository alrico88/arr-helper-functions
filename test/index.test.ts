import {areInAnyArray, areInArray, areInSameArray, distinctValues, FastArraySearcher, getBestSortingFunction, isInAnyArray, isInArray, sortArray} from '../src/';
...
describe('Test multiple elements finding methods', () => {
  ...
  test('areInAnyArray should return true if all elements are found in some array', () => {
    expect(areInAnyArray(['hello', 'world'], [['hello'], ['testing', 'this'], ['world']])).toBe(true);
  });
  ...
  test('areInSameArray should return true if all elements are found in same array', () => {
    expect(areInSameArray(['hello', 'world'], [['hello'], ['testing', 'this', 'hello', 'world']])).toBe(true);
  });
...
describe('Test fast finding methods', () => {
  ...
  test('Fast finder should accept as indexer a function as well as for a value getter', () => {
    ...
    const fastFinder = new FastArraySearcher(testArray, indexGetter, (d: TestObj): string => `${d['position']}_${d['value']}`);
  ...
  test('Fast finder should throw an error if the value used as indexer is not a string', () => {
    ...
    expect(() => {
      ...
      new FastArraySearcher(testArray, indexGetter, (d: TestObj): string => `${d['position']}_${d['value']}`);
    }).toThrow();
  });
});
...
describe('Test distinct values functions', () => {
  test('Unique finder should return no repeated elements', () => {
    expect(distinctValues(['this', 'is', 'repeated', 'repeated'])).toStrictEqual(['this', 'is', 'repeated']);
  });
});
